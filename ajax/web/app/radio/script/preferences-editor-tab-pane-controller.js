...
class PreferencesEditorTabPaneController extends TabPaneController {
	...
	async #displayPerson (person) {
		this.preferencesEditorAvatarViewer.src = RADIO_SERVICE.documentsURI + "/" + person.attributes["avatar-reference"];
	...
	async processSubmitAvatar (person, avatarFile) {
		try {
			...
			person.attributes["avatar-reference"] = await RADIO_SERVICE.insertOrUpdateDocument(avatarFile);
			this.preferencesEditorAvatarViewer.src = RADIO_SERVICE.documentsURI + "/" + person.attributes["avatar-reference"];
	...
	async processSubmitPerson (person) {
		try {
			...
			await RADIO_SERVICE.insertOrUpdatePerson(personClone, password);
			this.messageOutput.value = "ok.";
