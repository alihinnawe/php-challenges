...
async queryPeople (pagingOffset, pagingLimit, minCreated, maxCreated, minModified, maxModified, email, group, title, surname, forename, street, city, country, postcode) {
    ...
    if (pagingOffset != null) parameters.append("paging-offset", pagingOffset);
    if (pagingLimit != null) parameters.append("paging-limit", pagingLimit);
    if (minCreated != null) parameters.append("min-created", minCreated);
    if (maxCreated != null) parameters.append("max-created", maxCreated);
    if (minModified != null) parameters.append("min-modified", minModified);
    if (maxModified != null) parameters.append("max-modified", maxModified);
    if (email != null) parameters.append("email", email);
    if (group != null) parameters.append("group", group);
    if (title != null) parameters.append("title", title);
    if (surname != null) parameters.append("surname", surname);
    if (forename != null) parameters.append("forename", forename);
    if (street != null) parameters.append("street", street);
    if (city != null) parameters.append("city", city);
    if (country != null) parameters.append("country", country);
    if (postcode != null) parameters.append("postcode", postcode);
    
    const resource = this.#origin + "/services/people";
    const url = parameters.toString() ? resource + "?" + parameters.toString() : resource;
    const headers = { "Accept": "application/json" };
...
async findPerson (personIdentity) {
    const resource = this.#origin + "/services/people/" + encodeURIComponent(personIdentity);
    const headers = { "Accept": "application/json" };
...
async queryEditableRecipes (person, pagingOffset, pagingLimit) {
    ...
    // Determine the correct endpoint based on person's group (ADMIN vs USER)
    let resource;
    if (person.group === "ADMIN") {
        resource = this.#origin + "/services/recipes";
    } else {
        resource = this.#origin + "/services/people/" + person.identity + "/recipes";
    }
    
    const url = parameters.toString() ? resource + "?" + parameters.toString() : resource;
    const headers = { 
        "Accept": "application/json",
        "X-Requester-Identity": this.#requesterIdentity
    };
